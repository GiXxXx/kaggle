...
train_data = pd.read_csv(
    "~/kaggle-workspace/kaggle/digital-recognizer/train.csv")
test_data = pd.read_csv(
    "~/kaggle-workspace/kaggle/digital-recognizer/test.csv")
...
xgb_clf = XGBClassifier(max_depth=5, learning_rate=0.3, n_estimators=100,n_jobs=-1,random_state=34)

st_clf = StackingClassifier(classifiers=[knn_clf, rf_clf, xgb_clf], meta_classifier=LogisticRegression(
...
# for clf, label in zip([xgb_clf],
#                       [
#         'XGB']):

#     scores = model_selection.cross_val_score(
#         clf, train_data_pca, train_data.values[:, 0], cv=2, scoring='accuracy')
#     print("Accuracy: %0.2f (+/- %0.2f) [%s]" %
#           (scores.mean(), scores.std(), label))

st_clf.fit(train_data_pca, train_data.values[:, 0])

ret = st_clf.predict(test_data.values)
